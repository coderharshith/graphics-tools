...
class EditingTools:
    ...
    def blur_background(self, image: Image.Image, radius: int = 15) -> Image.Image:
    ...
    def change_bg_color(self, image: Image.Image, color: tuple = (255, 255, 255)) -> Image.Image:
        arr = np.array(self._ensure_rgb(image))
        bgr = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
        fg_mask = self._grabcut_mask(bgr)
        bg_mask = 255 - fg_mask
    ...
    def face_retouch(self, image: Image.Image) -> Image.Image:
        arr = np.array(self._ensure_rgb(image))
        bgr = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
    ...
    def portrait_enhance(self, image: Image.Image) -> Image.Image:
        arr = np.array(self._ensure_rgb(image))
        bgr = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
        ...
        smoothed = cv2.bilateralFilter(sharpened, 5, 60, 60)
    ...
    def skin_smooth(self, image: Image.Image, strength: float = 0.5) -> Image.Image:
        arr = np.array(self._ensure_rgb(image))
        bgr = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
    ...
    def teeth_whiten(self, image: Image.Image) -> Image.Image:
        arr = np.array(self._ensure_rgb(image))
        bgr = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
        hsv = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)
        ...
        wb[:, :, 2] = np.clip(wb[:, :, 2].astype(np.float32) * 0.97, 0, 255)
        wb[:, :, 0] = np.clip(wb[:, :, 0].astype(np.float32) * 1.03, 0, 255)
        mask_3 = np.stack([soft] * 3, axis=-1)
        result = bgr.astype(np.float32) * (1 - mask_3) + wb.astype(np.float32) * mask_3
        return Image.fromarray(np.clip(result, 0, 255).astype(np.uint8))

    def eye_enhance(self, image: Image.Image) -> Image.Image:
        arr = np.array(self._ensure_rgb(image))
        bgr = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
        gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
        ...
        for (x, y, ew, eh) in eyes:
            ...
            enhanced = cv2.cvtColor(lab, cv2.COLOR_LAB2BGR)
            sharpened = self._unsharp_mask(enhanced, 1.5, 1.2)
            bright = cv2.convertScaleAbs(sharpened, alpha=1.15, beta=0)
            cy, cx = eh // 2, ew // 2
            radius = max(ew, eh) // 2
            Y, X = np.ogrid[:eh, :ew]
            dist = np.sqrt((X - cx) ** 2 + (Y - cy) ** 2)
            cmask = np.clip(1.0 - dist / radius, 0, 1)
            cmask = cv2.GaussianBlur(cmask.astype(np.float32), (11, 11), 3)
            m3 = np.stack([cmask] * 3, axis=-1)
            blended = (roi.astype(np.float32) * (1 - m3) + bright.astype(np.float32) * m3).astype(np.uint8)
            result[y1:y2, x1:x2] = blended
        return Image.fromarray(cv2.cvtColor(result, cv2.COLOR_BGR2RGB))

    def ai_relight(self, image: Image.Image, direction: str = "center", intensity: float = 1.0) -> Image.Image:
        arr = np.array(self._ensure_rgb(image)).astype(np.float32)
        h, w = arr.shape[:2]
        y_coords, x_coords = np.mgrid[0:h, 0:w].astype(np.float32)
        cx, cy = w / 2, h / 2
        offsets = {
            "center": (0, 0), "top": (0, -h / 3), "bottom": (0, h / 3),
            "left": (-w / 3, 0), "right": (w / 3, 0),
        }
    ...
    def add_shadow(self, image: Image.Image, direction: str = "bottom-right", opacity: float = 0.3) -> Image.Image:
    ...
    def add_reflection(self, image: Image.Image, opacity: float = 0.4) -> Image.Image:
        ...
        sep = 2
        canvas = Image.new("RGB", (w, h * 2 + sep), (255, 255, 255))
        canvas.paste(img, (0, 0))
        canvas.paste(ref_final, (0, h + sep))
        return canvas
