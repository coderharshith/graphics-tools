...
def _dispatch(tool_id):
    if tool_id in TOOL_DISPATCH:
        return TOOL_DISPATCH[tool_id]
    return None
...
def _render_settings(tool_config):
    settings = tool_config.get("settings", {})
    values = {}
    for key, cfg in settings.items():
        stype = cfg.get("type", "text")
        if stype == "slider":
            values[key] = st.slider(cfg.get("label", key), min_value=cfg.get("min", 0),
                                    max_value=cfg.get("max", 100), value=cfg.get("default", 0),
                                    step=cfg.get("step", 1), key=f"set_{key}")
        elif stype == "select":
            values[key] = st.selectbox(cfg.get("label", key), cfg.get("options", []),
                                        key=f"set_{key}")
        elif stype == "color":
            values[key] = st.color_picker(cfg.get("label", key), value=cfg.get("default", "#ffffff"),
                                           key=f"set_{key}")
        elif stype == "number":
            values[key] = st.number_input(cfg.get("label", key), min_value=cfg.get("min", 0),
                                           value=cfg.get("default", 0), step=cfg.get("step", 1),
                                           key=f"set_{key}")
        elif stype == "text":
            values[key] = st.text_input(cfg.get("label", key), value=cfg.get("default", ""),
                                         key=f"set_{key}")
        elif stype == "textarea":
            values[key] = st.text_area(cfg.get("label", key), value=cfg.get("default", ""),
                                        key=f"set_{key}")
        elif stype == "checkbox":
            values[key] = st.checkbox(cfg.get("label", key), value=cfg.get("default", False),
                                       key=f"set_{key}")
        elif stype == "radio":
            values[key] = st.radio(cfg.get("label", key), cfg.get("options", []),
                                    key=f"set_{key}")
    return values
...
def _process_tool(tool_config, images, settings):
    ...
    if tid == "ai_enhancer":
        for img in images:
            ...
            results.append({"image": out, "name": f"enhanced_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "hd_enhancer":
        for img in images:
            out = ai_tools.hd_enhance(img, strength=settings.get("strength", 1.5))
            results.append({"image": out, "name": f"hd_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "upscale_4k":
        for img in images:
            out = ai_tools.upscale_4k(img)
            results.append({"image": out, "name": f"4k_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "upscale_8k":
        for img in images:
            out = ai_tools.upscale_8k(img)
            results.append({"image": out, "name": f"8k_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "object_remover":
        for img in images:
            out = ai_tools.remove_object(img)
            results.append({"image": out, "name": f"cleaned_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "watermark_remover":
        for img in images:
            out = ai_tools.remove_watermark(img)
            results.append({"image": out, "name": f"nowm_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "text_remover":
        for img in images:
            out = ai_tools.remove_text(img)
            results.append({"image": out, "name": f"notext_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "bg_blur":
        for img in images:
            out = image_editor.blur_background(img, radius=settings.get("radius", 15))
            results.append({"image": out, "name": f"blurred_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
        ...
        for img in images:
            ...
            results.append({"image": out, "name": f"bgchanged_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "face_retouch":
        for img in images:
            out = image_editor.face_retouch(img)
            results.append({"image": out, "name": f"retouched_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "portrait_enhance":
        for img in images:
            out = image_editor.portrait_enhance(img)
            results.append({"image": out, "name": f"portrait_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "skin_smooth":
        for img in images:
            out = image_editor.skin_smooth(img, strength=settings.get("strength", 0.5))
            results.append({"image": out, "name": f"smooth_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "teeth_whiten":
        for img in images:
            out = image_editor.teeth_whiten(img)
            results.append({"image": out, "name": f"white_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "eye_enhance":
        for img in images:
            out = image_editor.eye_enhance(img)
            results.append({"image": out, "name": f"eyes_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "ai_relight":
        for img in images:
            out = image_editor.ai_relight(img, light_direction=settings.get("direction", "center"),
                                           intensity=settings.get("intensity", 1.0))
            results.append({"image": out, "name": f"relit_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "shadow_gen":
        for img in images:
            out = image_editor.add_shadow(img, direction=settings.get("direction", "bottom-right"),
                                          opacity=settings.get("opacity", 0.3))
            name = f"shadow_{len(results)}.png"
            results.append({"image": out.convert("RGB") if out.mode == "RGBA" else out,
                            "name": name, "bytes": pil_to_bytes(out.convert("RGBA") if out.mode != "RGBA" else out)})
    elif tid == "reflection_gen":
        for img in images:
            out = image_editor.add_reflection(img, opacity=settings.get("opacity", 0.4))
            results.append({"image": out, "name": f"reflect_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "bg_remover":
        for img in images:
            out = bg_remover.remove(img)
            results.append({"image": out.convert("RGB") if out.mode == "RGBA" else out,
                            "name": f"nobg_{len(results)}.png",
                            "bytes": pil_to_bytes(out.convert("RGBA") if out.mode != "RGBA" else out, fmt="PNG")})
    elif tid == "bg_adder":
        ...
        for img in images:
            ...
            out = bg_adder.add_solid(rgba, color=color) if bg_type == "Solid Color" else bg_adder.add_solid(rgba, color=color)
            results.append({"image": out, "name": f"bgadded_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
        ...
        for img in images:
            ...
            results.append({"image": out, "name": f"graded_{len(results)}.{ext}",
                            "bytes": pil_to_bytes(out, fmt=fmt)})
    elif tid == "cropper":
        for img in images:
            ...
            results.append({"image": out, "name": f"cropped_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "resizer":
        for img in images:
            ...
            results.append({"image": out, "name": f"resized_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "rotator":
        for img in images:
            out = image_utils.rotate_image(img, angle=settings.get("angle", 0))
            results.append({"image": out, "name": f"rotated_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "flipper":
        for img in images:
            out = image_utils.flip_image(img, direction=settings.get("direction", "horizontal"))
            results.append({"image": out, "name": f"flipped_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "poster":
        ...
        for img in images:
            ...
            results.append({"image": out, "name": f"meme_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
    elif tid == "collage":
        ...
        for img in images:
            if op == "Enhance":
                ...
                out = img
            results.append({"image": out, "name": f"batch_{len(results)}.png",
                            "bytes": pil_to_bytes(out)})
...
def render_tool_workspace(tool_config):
    ...
    if tool_config.get("settings"):
        with st.expander("Settings", expanded=True):
            settings = _render_settings(tool_config)
    ...
    if can_process:
        if st.button("Process", type="primary", use_container_width=True, key="process_btn"):
            with st.spinner("Processing..."):
                try:
                    if accepted and images:
                        results = _process_tool(tool_config, images, settings)
                    else:
                        results = _process_tool(tool_config, [], settings)
